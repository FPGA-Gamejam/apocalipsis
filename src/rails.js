...
function rails(x, y) {
	if (!rail) {
		rail = level3_svg.layer("rails");
	}
	var closest = false;
	var distance;
	rail.forEach(function(obj) {
		switch (obj.type) {
			case "path":
				var point;
				if (obj.vertices.length >= 2) {
					for (var i = 0; i != obj.vertices.length - 1; i++) {
						var point = closestPolyLinePoint(x, y, obj.vertices[i][0], obj.vertices[i][1], obj.vertices[i + 1][0], obj.vertices[i + 1][1]);
						var d = dist(point.x, point.y, x, y)
